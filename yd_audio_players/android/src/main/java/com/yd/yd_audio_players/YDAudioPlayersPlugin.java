...
public class YDAudioPlayersPlugin implements MethodCallHandler {

    private static final Logger LOGGER = Logger.getLogger(YDAudioPlayersPlugin.class.getCanonicalName());
    ...
    public static void registerWith(final Registrar registrar) {
        final MethodChannel channel = new MethodChannel(registrar.messenger(), "Netease-YD/audioplayers");
        channel.setMethodCallHandler(new YDAudioPlayersPlugin(channel, registrar.activity()));
    }

    private YDAudioPlayersPlugin(final MethodChannel channel, Activity activity) {
    ...
    private void handleMethodCall(final MethodCall call, final MethodChannel.Result response) {
        ...
        switch (call.method) {
            case "play": {
                ...
                player.configAttributes(respectSilence, stayAwake == null ? true : false, activity.getApplicationContext());
    ...
    private static final class UpdateCallback implements Runnable {
        ...
        private final WeakReference<YDAudioPlayersPlugin> audioplayersPlugin;

        private UpdateCallback(final Map<String, Player> mediaPlayers,
                               final MethodChannel channel,
                               final Handler handler,
                               final YDAudioPlayersPlugin audioplayersPlugin) {
        ...
        public void run() {
            ...
            final YDAudioPlayersPlugin audioplayersPlugin = this.audioplayersPlugin.get();
